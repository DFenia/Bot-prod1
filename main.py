...
bot = telebot.TeleBot('5433904271:AAFfV9mHZvVxmzXdwcmc4ywmddPznXIbnwU')
...
def command(message, res=False):
    ...
    txt_message = f"Привет, {message.from_user.first_name}! Я Бот по продаже! Выберите то, что вам нужно:"
...
def get_text_messages(message):
    ...
    if cur_menu != None and ms_text in cur_menu.buttons:  # проверим, что команда относится к текущему меню
        ...
        if ms_text == "Контакты админа":
            send_help(chat_id)

        # ======================================= Развлечения(число для себя)
