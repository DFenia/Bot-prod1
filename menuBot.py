...
m_main = Menu("Главное меню", buttons=["Развлечения(чисто для себя)", "Предпоказ", "Товары", "Контакты админа"])

m_DZ = Menu("Предпоказ", buttons=["Фото-1", "Фото-2", "Фото-3", "Выход"], parent=m_main)

m_fun = Menu("Развлечения(чисто для себя)", buttons=["Прислать собаку", "Прислать котика", "Человек и email", "Прислать анекдот", "Создать пароль", "Прислать игру", "Выход"], parent=m_main)

m_VS = Menu("Товары", buttons=["Фото", "Короткие видео", "Длинные видео", "test" , "Выход"], parent=m_main)

m_FF = Menu("test", buttons=["1","2", "Выход"], parent=m_VS)
